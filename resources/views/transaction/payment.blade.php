        ...
        <div class="card-body">
            <form method="POST" action="{{ route('transaction.processPayment', ['id' => $transaction->id_transaksi]) }}">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            <label for="discount_ps">Diskon Rental PS (%)</label>
                            <div class="input-group">
                                <input type="number" class="form-control @error('discount_ps') is-invalid @enderror" id="discount_ps" name="discount_ps" min="0" max="100" value="0" step="0.1" oninput="updateTotalAfterDiscount()">
                                <div class="input-group-append">
                                    <span class="input-group-text">%</span>
                                </div>
                            </div>
                            <small class="form-text text-muted">Diskon untuk biaya rental PlayStation</small>
                            @error('discount_ps')
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            <label for="discount_fnb">Diskon FnB (%)</label>
                            <div class="input-group">
                                <input type="number" class="form-control @error('discount_fnb') is-invalid @enderror" id="discount_fnb" name="discount_fnb" min="0" max="100" value="0" step="0.1" oninput="updateTotalAfterDiscount()">
                                <div class="input-group-append">
                                    <span class="input-group-text">%</span>
                                </div>
                            </div>
                            <small class="form-text text-muted">Diskon untuk makanan dan minuman</small>
                            @error('discount_fnb')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
<script>
    ...
    const psBiaya = {{ $transaction->total - $transaction->getFnbTotalAttribute() }};
    const fnbBiaya = {{ $transaction->getFnbTotalAttribute() }};
    let finalTotal = originalTotal;
    
    function updateTotalAfterDiscount() {
        const discountPsPercentage = parseFloat(document.getElementById('discount_ps').value) || 0;
        const discountFnbPercentage = parseFloat(document.getElementById('discount_fnb').value) || 0;
        
        const discountPsAmount = (psBiaya * discountPsPercentage) / 100;
        const discountFnbAmount = (fnbBiaya * discountFnbPercentage) / 100;
        const totalDiscountAmount = discountPsAmount + discountFnbAmount;
        ...
        // Update final total display only if any discount > 0
